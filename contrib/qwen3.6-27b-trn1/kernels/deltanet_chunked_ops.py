...
Layout (per TP core, row stream*S+t; stream flattens batch and head):
  state:  [STREAMS*K_DIM, V_DIM]  f32
  q,k:    [STREAMS*S, K_DIM]      f32 (RAW, un-normed)
  v:      [STREAMS*S, V_DIM]      f32
  g,beta: [STREAMS*S, 1]          f32
  m_incl/m_strict/eye: [C, C]     f32 (host constants; no iota in this NKI build)
Returns (output[STREAMS*S,V_DIM] f32 raw pre-gate,
...
@nki_op("deltanet::chunked_prefill", mutates_args={})
def deltanet_chunked_prefill(
    state: torch.Tensor,      # [STREAMS*K_DIM, V_DIM] f32
    query: torch.Tensor,      # [STREAMS*S, K_DIM] f32 (raw)
    key: torch.Tensor,        # [STREAMS*S, K_DIM] f32 (raw)
    value: torch.Tensor,      # [STREAMS*S, V_DIM] f32
    g: torch.Tensor,          # [STREAMS*S, 1] f32
    beta: torch.Tensor,       # [STREAMS*S, 1] f32
    m_incl: torch.Tensor,     # [C, C] f32
    m_strict: torch.Tensor,   # [C, C] f32
    eye: torch.Tensor,        # [C, C] f32
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return raw output and final state for all flattened batch/head streams."""
