...
# The full and batched decode DeltaNet kernels are numerically wrong at TP=8/H=6.
# DECODE_VIA_CHUNKED=1 routes the complete batch through the validated
# chunked-prefill NKI recurrence in one custom call per layer.
USE_DECODE_VIA_CHUNKED = os.environ.get("DECODE_VIA_CHUNKED", "0") == "1"
# DECODE_SEGMENTED=1 is the correctness-first eager fallback: it inserts an
# xm.mark_step() between decode layers so each layer executes as its own lazy
# sub-graph. The faster validated path is DECODE_COMPILE_STEP=1 below.
...
# cutting graph-break/sync overhead. Only active when the matching *_SEGMENTED flag is on.
...
# A2: compile the decode step into this many coarse segments while leaving the
# wrapper eager. Kept for diagnostics; the eager wrapper recompiles per token on
# PJRT, so DECODE_COMPILE_STEP is the production path.
DECODE_COMPILE_SPLITS = int(os.environ.get("DECODE_COMPILE_SPLITS", "0"))
# Compile embed -> layer loop -> final norm -> LM head -> greedy argmax as one
# torch.compile entry point. With fullgraph=False, DECODE_STEP_BREAK_EVERY splits
# the layer loop into cached subgraphs while keeping the wrapper out of eager lazy
# mode. Validated coherent on trn1 TP8 with fullgraph=True: 42.7 ms/token and
# zero recompiles after the first call. Default off preserves existing callers.
USE_DECODE_COMPILE_STEP = os.environ.get("DECODE_COMPILE_STEP", "0") == "1"
DECODE_STEP_FULLGRAPH = os.environ.get("DECODE_STEP_FULLGRAPH", "1") == "1"
DECODE_STEP_BREAK_EVERY = max(
    0, int(os.environ.get("DECODE_STEP_BREAK_EVERY", "0"))
)
# Profile-guided BS=1 experiment: persist the MLP and LM-head weights as
# contiguous [K,N] tensors and call torch.matmul(x, weight_kn) directly. This
# removes F.linear's logical weight transpose from the graph and tests whether
# the compiler can avoid its transpose-heavy GEMV mapping. On trn1 TP8 this
# reduced coherent fullgraph decode from 42.7 to 39.9 ms/token. Default off.
USE_BF16_PREPACKED_LINEAR = (
    os.environ.get("BF16_PREPACKED_LINEAR", "0") == "1"
)

if USE_DECODE_COMPILE_STEP and DECODE_COMPILE_SPLITS > 0:
    raise SystemExit(
        "DECODE_COMPILE_STEP=1 and DECODE_COMPILE_SPLITS>0 are mutually exclusive."
    )
if USE_DECODE_COMPILE_STEP and USE_DECODE_SEGMENTED:
    raise SystemExit(
        "DECODE_COMPILE_STEP=1 and DECODE_SEGMENTED=1 are mutually exclusive; "
        "use DECODE_STEP_BREAK_EVERY with DECODE_STEP_FULLGRAPH=0."
    )
if (
    USE_DECODE_COMPILE_STEP
    and DECODE_STEP_FULLGRAPH
    and DECODE_STEP_BREAK_EVERY > 0
):
    raise SystemExit(
        "DECODE_STEP_BREAK_EVERY requires DECODE_STEP_FULLGRAPH=0."
    )
...
class StaticDecodeModule(nn.Module):
    ...
    def __init__(self, weights: dict, max_seq_len: int, world_size: int,
        ...
        super().__init__()
        if fp8_weights and USE_BF16_PREPACKED_LINEAR:
            raise ValueError(
                "BF16_PREPACKED_LINEAR=1 cannot be combined with --fp8-weights."
            )
        ...
        # Names of bf16 buffers persisted as contiguous [in, out] = [K,N].
        # _lin routes these through torch.matmul rather than F.linear.
        self._prepacked_linear_w: set[str] = set()
        ...
        def reg_linear(name: str, w: torch.Tensor, src: dict, src_key: str,
            ...
            if self.fp8_weights:
                ...
                # Replace [out,in] with one contiguous [in,out] copy. The source
                # entry is removed below, so prepacking does not retain both layouts.
                self.register_buffer(name, w.t().contiguous())
                self._prepacked_linear_w.add(name)
            else:
        ...
        reg_linear(
            "lm_head_w", weights["lm_head"], weights, "lm_head",
            prepack=USE_BF16_PREPACKED_LINEAR,
        )
        ...
        for i, lw in enumerate(weights["layers"]):
            ...
            reg_linear(
                f"l{i}_mlp_gate", lw["mlp_gate"], lw, "mlp_gate",
                prepack=USE_BF16_PREPACKED_LINEAR,
            )
            reg_linear(
                f"l{i}_mlp_up", lw["mlp_up"], lw, "mlp_up",
                prepack=USE_BF16_PREPACKED_LINEAR,
            )
            reg_linear(
                f"l{i}_mlp_down", lw["mlp_down"], lw, "mlp_down",
                prepack=USE_BF16_PREPACKED_LINEAR,
            )
        ...
        expected_prepacked = 3 * NUM_LAYERS + 1
        if USE_BF16_PREPACKED_LINEAR:
            assert len(self._prepacked_linear_w) == expected_prepacked
        else:
            assert not self._prepacked_linear_w
    ...
    def _lin(self, name: str, x: torch.Tensor) -> torch.Tensor:
        """Linear by name, selecting FP8, prepacked bf16, or F.linear."""
        if self.fp8_weights:
        ...
        w = getattr(self, name)
        x = x.to(w.dtype)
        if name in self._prepacked_linear_w:
    ...
    def setup_decode_segments(self, n_splits: int):
        ...
        the monolithic graph. forward() dispatches through the
    ...
    def _decode_hidden(
        ...
        # single-NEFF perf, but leaves the PJRT-recompiling wrapper eager.
        ...
        elif USE_DECODE_COMPILE_STEP and DECODE_STEP_BREAK_EVERY > 0:
            for lo in range(0, NUM_LAYERS, DECODE_STEP_BREAK_EVERY):
                hi = min(lo + DECODE_STEP_BREAK_EVERY, NUM_LAYERS)
    ...
    def decode_step(
        ...
        """Decode, select the next token, and advance position in one entry point."""
        hidden, dn_states, cv_states, kv_k, kv_v = self._decode_hidden(
            input_id, position, deltanet_states, conv_states, kv_cache_k, kv_cache_v
        )
        logits = self._lm_head_logits(hidden.to(torch.bfloat16)).squeeze(1)
        # torch-neuronx 2.9 overrides argmax with an XLA user computation that
        # rejects Dynamo FakeTensors. max.dim has the same greedy/tie semantics
        # and traces through openxla, so keep token selection inside this graph.
        next_id = logits.max(dim=-1).indices.to(torch.long)
        next_position = position + 1
        return logits, next_id, next_position, dn_states, cv_states, kv_k, kv_v
    ...
    def _deltanet_decode_chunked(self, i: int, x_2d: torch.Tensor,
                                 dn_states: torch.Tensor, cv_states: torch.Tensor):
        """Decode DeltaNet via the validated chunked_prefill NKI kernel.

        Projections and causal convolution remain batched. Batch and head are
        flattened into independent kernel streams, so the recurrent state is
        updated by one opaque custom call rather than one call and scatter per
        batch row. The real token is followed by state-preserving padding when
        DECODE_CHUNK exceeds one.
        """
        di = deltanet_index(i)
        ...
        mixed_qkv = self._lin(f"l{i}_dn_qkv", x_2d)          # [B, QKV]
        a_out = self._lin(f"l{i}_dn_a", x_2d).float()        # [B, H]
        b_out = self._lin(f"l{i}_dn_b", x_2d)                # [B, H]
        z = self._lin(f"l{i}_dn_z", x_2d).reshape(B, H, DN_V_DIM)

        cur = mixed_qkv.unsqueeze(-1)                       # [B, QKV, 1]
        conv_in = torch.cat([cv_states[di], cur], dim=-1)  # [B, QKV, 4]
        conv_out = F.conv1d(conv_in, conv_w, groups=DN_QKV_DIM)
        cv_states[di] = conv_in[:, :, 1:].to(cv_states.dtype)
        mixed = _silu(conv_out.squeeze(-1))                 # [B, QKV]

        q = mixed[:, :DN_K_HEADS * DN_K_DIM].reshape(
            B, DN_K_HEADS, DN_K_DIM
        )
        k = mixed[
            :, DN_K_HEADS * DN_K_DIM:2 * DN_K_HEADS * DN_K_DIM
        ].reshape(B, DN_K_HEADS, DN_K_DIM)
        v = mixed[:, 2 * DN_K_HEADS * DN_K_DIM:].reshape(
            B, H, DN_V_DIM
        )
        q = q.repeat_interleave(H // DN_K_HEADS, dim=1)
        k = k.repeat_interleave(H // DN_K_HEADS, dim=1)

        beta = b_out.sigmoid().float()                      # [B, H]
        g = -A_log.float().exp() * F.softplus(
            a_out + dt_bias.float()
        )                                                   # [B, H]

        # Batch-major stream layout: row ((batch*H + head)*C + token).
        q_hm = q.float().unsqueeze(2)
        k_hm = k.float().unsqueeze(2)
        v_hm = v.float().unsqueeze(2)
        g_hm = g.unsqueeze(-1)
        beta_hm = beta.unsqueeze(-1)
        if C > 1:
            q_hm = F.pad(q_hm, (0, 0, 0, C - 1))
            k_hm = F.pad(k_hm, (0, 0, 0, C - 1))
            v_hm = F.pad(v_hm, (0, 0, 0, C - 1))
            g_hm = F.pad(g_hm, (0, C - 1))
            beta_hm = F.pad(beta_hm, (0, C - 1))
        state_in = dn_states[di].float().reshape(
            B * H * DN_K_DIM, DN_V_DIM
        ).contiguous()

        out_hm, new_state = torch.ops.deltanet.chunked_prefill(
            state_in,
            q_hm.reshape(B * H * C, DN_K_DIM).contiguous(),
            k_hm.reshape(B * H * C, DN_K_DIM).contiguous(),
            v_hm.reshape(B * H * C, DN_V_DIM).contiguous(),
            g_hm.reshape(B * H * C, 1).contiguous(),
            beta_hm.reshape(B * H * C, 1).contiguous(),
            self.dchunk_m_incl,
            self.dchunk_m_strict,
            self.dchunk_eye,
        )
        dn_states[di] = new_state.reshape(
            B, H * DN_K_DIM, DN_V_DIM
        ).to(dn_states.dtype)
        attn_out = out_hm.reshape(B, H, C, DN_V_DIM)[:, :, 0, :]
        gated = rms_norm_gated(attn_out.to(x_2d.dtype), z, norm_w)
        out = self._lin(
            f"l{i}_dn_out",
            gated.reshape(B, DN_VALUE_DIM).to(torch.bfloat16),
        )
        out = functional_all_reduce(out, "sum", self.tp_group)
    ...
    def _deltanet_layer(
        ...
        """DeltaNet layer for decode (seq_len=1, batch B).

        DECODE_VIA_CHUNKED uses the coherent batched recurrence. Otherwise,
        B=1 uses deltanet.full and B>1 uses deltanet.full_batched.
        """
        di = deltanet_index(i)
        B = self.batch_size
        ...
        # TP8 coherence path: route the batch through the validated chunked kernel.
        if USE_DECODE_VIA_CHUNKED and not self.fp8_weights:
...
def main():
    ...
    if args.fp8_weights and USE_BF16_PREPACKED_LINEAR:
        raise SystemExit(
            "BF16_PREPACKED_LINEAR=1 cannot be combined with --fp8-weights."
        )
    ...
    # 6. Compile
    if USE_DECODE_COMPILE_STEP:
        if args.skip_compile:
            raise SystemExit(
                "DECODE_COMPILE_STEP=1 cannot be combined with --skip-compile."
            )
        if rank == 0:
            print(
                f"  [experimental] Compiling whole decode_step with "
                f"backend='{_COMPILE_BACKEND}', "
                f"fullgraph={DECODE_STEP_FULLGRAPH}, "
                f"break_every={DECODE_STEP_BREAK_EVERY}..."
            )
        t0 = time.time()
        static_module.decode_step = torch.compile(
            static_module.decode_step,
            backend=_COMPILE_BACKEND,
            fullgraph=DECODE_STEP_FULLGRAPH,
            dynamic=False,
        )
        if rank == 0:
            print(f"  decode_step torch.compile setup: {time.time()-t0:.1f}s")
    elif DECODE_COMPILE_SPLITS > 0:
        # A2 diagnostic: compile coarse spans while forward stays eager.
        t0 = time.time()
    ...
    def run_decode(input_id, position, dns, cvs, keys, values):
        if USE_DECODE_COMPILE_STEP:
            return static_module.decode_step(
                input_id, position, dns, cvs, keys, values
            )
        outputs = static_module(input_id, position, dns, cvs, keys, values)
        logits = outputs[0]
        next_id = logits.argmax(dim=-1).to(torch.long)
        next_position = position + 1
        return logits, next_id, next_position, *outputs[1:]
    ...
    if profile_steps > 0:
        ...
        with torch.no_grad():
            for _ in range(3):  # warmup (not traced meaningfully)
                logits, _, _, dn_states, conv_states, kv_k, kv_v = run_decode(
    ...
    with torch.no_grad():
        ...
        pos = torch.tensor(start_pos, dtype=torch.long, device=device)
        for _ in range(3):  # warm up
            logits, nid, pos, dn_states, conv_states, kv_k, kv_v = run_decode(
