#!/usr/bin/env bash
# Barrier/path discriminator for the >2048-token CTE MoE deadlock. Same clean-flag
# runner as run_prefill_fp8_moecap.sh, plus the two route-packer overrides that
# break the confound at the threshold: at 2048 tokens/call the packer switches
# direct -> tiled stable scan AND scatter_barrier goes True -> False together.
#   BARRIER=1 forces the per-slice scatter core barriers ON on the tiled path.
#   TILED=1   forces the tiled stable scan at any call size.
# NO debug flags (LAUNCH_BLOCKING / DGE notifications cost ~15x).
# usage: run_prefill_fp8_barrier.sh [bs] [n] [fp8] [layers] [splits] [bucket] [blk] [maxtok] [barrier] [tiled]
set -uo pipefail
BS="${1:-2}"; N="${2:-1024}"; FP8="${3:-1}"; LAYERS="${4:-4}"; SPLITS="${5:-2}"
BUCKET="${6:-1024}"; BLK="${7:-512}"; MAXTOK="${8:-2048}"
BARRIER="${9:-}"; TILED="${10:-}"; ABLATE="${11:-}"
...
TAG=bs${BS}-n${N}-l${LAYERS}-s${SPLITS}-bc${BUCKET}-blk${BLK}-mt${MAXTOK}-fp8${FP8}-br${BARRIER:-d}-tl${TILED:-d}-ab${ABLATE:-none}
...
echo "START $(date -u +%FT%TZ) tag=$TAG maxseq=$MAXSEQ blk=$BLK maxtok=$MAXTOK barrier=${BARRIER:-default} tiled=${TILED:-default} ablate=${ABLATE:-none}" | tee "$LOG"
docker run --rm --name "$NAME" --privileged --network host --ipc host \
  ...
  -e MOE_CTE_SCATTER_BARRIER="${BARRIER}" -e MOE_CTE_FORCE_TILED="${TILED}" \
  -e MOE_CTE_ABLATE="${ABLATE}" \
