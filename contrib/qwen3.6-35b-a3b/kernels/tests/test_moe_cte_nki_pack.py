...
def route_cases(tokens: int, expert_lo: int):
    ...
    duplicate_ids = (
        torch.arange(tokens, dtype=torch.int32) % 64 + expert_lo
    ).unsqueeze(1)
...
def run_metadata_matrix(backend: str):
    if backend == "simulation":
        ...
        def invoke(routes, expert_lo, block_size):
            return nki.simulate(nki_pack_local_routes_35b)[1](
                routes.numpy(), expert_lo, block_size
            )
        ...
        kernel = wrap_nki(nki_pack_local_routes_35b)[2]

        def invoke(routes, expert_lo, block_size):
            outputs = kernel(routes.to(device), expert_lo, block_size)
            torch.neuron.synchronize()
            return tuple(output.cpu() for output in outputs)

    checked = 0
    for tokens in (1024, 2048):
        for expert_lo in (0, 64, 128, 192):
            for block_size in (256, 512):
                for case_name, routes in route_cases(tokens, expert_lo):
                    expected = pack_local_routes(
                        routes, expert_lo, 64, block_size
                    )
    ...
    print(f"NKI route packer: {checked} exact metadata cases passed via {backend}")
...
def run_fused_equivalence():
    ...
    metadata = pack_local_routes(routes, expert_lo, 64, block_size)
    hidden = torch.randn(
        tokens + 1, hidden_size, generator=generator, dtype=torch.bfloat16
    )
    affinities = torch.randn(
        (tokens + 1) * 64, 1, generator=generator, dtype=torch.bfloat16
    )
    gate_up = torch.randn(
        64,
        hidden_size,
    ...
    precomputed = wrap_nki(nki_moe_cte_35b)[2]
    fused = wrap_nki(nki_moe_cte_routed_35b)[2]
...
def run_distributed_equivalence(
    ...
    if world_size != 4:
        raise RuntimeError(
            f"distributed route-pack check requires world size 4, found {world_size}"
        )
    ...
    metadata_d = tuple(tensor.to(device) for tensor in metadata)
    pack = wrap_nki(nki_pack_local_routes_35b)[2]
    precomputed = wrap_nki(nki_moe_cte_35b)[2]
    fused = wrap_nki(nki_moe_cte_routed_35b)[2]
    ...
    for iteration in range(iterations):
        if mode == "pack":
            actual = pack(routes_d, expert_lo, block_size)
            torch.neuron.synchronize()
...
def main():
    ...
    parser.add_argument(
        ...
        help="run only the selected concurrent TP=4 device check",
    )
