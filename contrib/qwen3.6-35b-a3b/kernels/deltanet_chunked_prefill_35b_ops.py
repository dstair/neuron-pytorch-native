...
Layout (per TP core, head-major: row h*S+t):
  ...
  q,k:    [B*V_HEADS*S, K_DIM]       f32 (RAW, batch/head-major)
  v:      [B*V_HEADS*S, V_DIM]       f32
  g,beta: [B*V_HEADS*S, 1]           f32
  m_incl/m_strict/eye: [C, C]     f32 (host constants; no iota in this NKI build)
Returns (output[B*V_HEADS*S,V_DIM] f32 raw pre-gate,
...
def deltanet_chunked_prefill(
    ...
    query: torch.Tensor,      # [B*V_HEADS*S,K_DIM] f32 (raw)
    key: torch.Tensor,        # [B*V_HEADS*S,K_DIM] f32 (raw)
    value: torch.Tensor,      # [B*V_HEADS*S,V_DIM] f32
    g: torch.Tensor,          # [B*V_HEADS*S,1] f32
    beta: torch.Tensor,       # [B*V_HEADS*S,1] f32
    m_incl: torch.Tensor,     # [C, C] f32
    m_strict: torch.Tensor,   # [C, C] f32
    eye: torch.Tensor,        # [C, C] f32
) -> tuple[torch.Tensor, torch.Tensor]:
    """Returns batched raw output and [B,V_HEADS*K_DIM,V_DIM] final state."""
