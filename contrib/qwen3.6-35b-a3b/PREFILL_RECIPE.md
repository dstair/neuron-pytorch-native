...
**≈3,457 aggregate prompt tok/s** at **TP=8/LNC=1** (§3c) — **+24.5%** over the
prior TP=4/LNC=2 record. Both use the identical packed-C32 DeltaNet kernel; LNC=1
...
budget, and the ~30-line `moe_cte` LNC=1 patch (§3c).

The TP=4/LNC=2 configuration (§3a) is **≈2,776 aggregate prompt tok/s** (BS=2,
N=20000 tokens, query bucket 1024, DeltaNet **stable C32** block-diagonal inverse
with **4-stream block-diagonal packing** and **SBUF-resident intermediates**,
fused NKI route packer, optlevel-1) — **+21.9%** over unpacked C32 (≈2,277 tok/s)
and **+32.8%** over the paired-C16 baseline (≈2,090). It remains the simplest to
reproduce (no vocab sharding, no nkilib patch) and is the reliable fallback.
...
|---|---:|---:|
| **Packed C32 ×4, TP=8/LNC=1** (§3c, vocab-sharded head+embed, pg256) | **11.572 s** | **3,456.8** |
| Packed C32 ×4, SBUF-resident + transpose-once finish, TP=4/LNC=2 (`DN_PACK_C32=1 DN_PACK_N=4`) | 14.411 s | 2,775.6 |
