"""
DeepSeek V4 inference on Neuron with TP=4/EP=16 hybrid parallelism.

TP=4 within each Neuron device (fast on-chip interconnect).
EP=16 across devices (each device owns 16 of 256 experts).
Attention all-reduces use TP group (ring size 4).
MoE all-reduces use EP group (ring size 16).

Usage:
    torchrun --nproc-per-node=64 run_deepseek_v4_hybrid.py \
        --model-path /scratch/DeepSeek-V4-Flash-BF16-TP4EP16
"""
...
class Attention(nn.Module):
    ...
    def forward(self, x, cos, sin, positions, mask=None, cache=None, cache_pos=None):
        ...
        first_global_head = tp_rank_local * local_heads
        ...
        # Slice for this rank's wo_b shard (sharded by TP)
        lora_total = self.o_groups * self.o_lora_rank
        flat = out_lora.flatten(2)  # (b, s, o_groups * o_lora_rank)
        tp_rank = rank % TP_SIZE
        chunk = lora_total // TP_SIZE if TP_SIZE > 1 else lora_total
        local_flat = flat[:, :, tp_rank * chunk : (tp_rank + 1) * chunk]
...
class MoE(nn.Module):
    ...
    def __init__(self, args: ModelArgs, layer_idx: int, rank: int, world_size: int):
        ...
        # Expert partition by EP rank (not global rank)
        ep_rank = rank // TP_SIZE
        self.experts_per_rank = args.local_n_experts
        self.rank_start = ep_rank * self.experts_per_rank
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, h, cos, sin, positions, mask, cache=None, cache_pos=None,
                comm_stream=None):
        # --- Attention: all-reduce over TP group (4 ranks, on-chip) ---
        residual = h
        ...
        if self.tp_size > 1 and comm_stream is not None:
            evt = torch.neuron.Event()
            with torch.neuron.stream(comm_stream):
                x_full = all_reduce(x_partial, "sum", self.tp_group)
        ...
        # --- MoE: all-reduce over EP group (16 ranks, cross-device) ---
        residual = h
        x, post, comb = self.hc_pre(
        ...
        if self.ep_size > 1 and comm_stream is not None:
            evt = torch.neuron.Event()
            with torch.neuron.stream(comm_stream):
                moe_full = all_reduce(moe_partial, "sum", self.ep_group)
                evt.record(comm_stream)
            term_b = (comb.unsqueeze(-1) * residual.unsqueeze(-2)).sum(dim=2)
            torch.neuron.default_stream().wait_event(evt)
...
class DeepSeekV4(nn.Module):
    def __init__(self, args: ModelArgs, rank: int, world_size: int,
        ...
        self.layers = nn.ModuleList(
            [DecoderLayer(args, i, rank, world_size, tp_group, ep_group)
...
def main():
    ...
    # Hybrid TP/EP topology
    tp_rank = rank % TP_SIZE
    ep_rank = rank // TP_SIZE
    # TP group: ranks within same device (consecutive)
    tp_group = list(range(ep_rank * TP_SIZE, (ep_rank + 1) * TP_SIZE))
    # EP group: one rank per device (strided by TP_SIZE)
    ep_group = list(range(tp_rank, world_size, TP_SIZE))

    if rank == 0:
        print(f"=== DeepSeek V4 Hybrid TP={TP_SIZE}/EP={EP_SIZE} Inference ===")
        print(f"World size: {world_size}")
        print(f"TP groups: {TP_SIZE} ranks (on-chip), EP groups: {EP_SIZE} ranks (cross-device)")
        if use_presharded:
    ...
    # Validate
    assert margs.n_heads % TP_SIZE == 0
    assert margs.n_routed_experts % EP_SIZE == 0
    assert world_size == TP_SIZE * EP_SIZE

    # Set local dimensions for hybrid sharding
    margs.local_n_heads = margs.n_heads // TP_SIZE  # 64/4 = 16
    margs.local_n_experts = margs.n_routed_experts // EP_SIZE  # 256/16 = 16
    # wo_b sharded by TP=4
    margs.local_o_groups = margs.o_groups  # wo_a replicated
    ...
    if use_presharded:
        with torch.device("meta"):
            model = DeepSeekV4(margs, rank, world_size, tp_group, ep_group)
    else:
        model = DeepSeekV4(margs, rank, world_size, tp_group, ep_group)
    if rank == 0:
