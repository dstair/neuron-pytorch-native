...
def nki_deltanet_full_batched(
    ...
    exp_g_hbm = nl.ndarray((B * V_HEADS, 1), dtype=nl.float32, buffer=_SCRATCH)
    beta_hbm = nl.ndarray((B * V_HEADS, 1), dtype=nl.float32, buffer=_SCRATCH)
    ...
    for b in nl.affine_range(B):
        ...
        for t in nl.affine_range(NUM_QKV_TILES):
            ...
            nisa.activation(dst=nq_f, op=nl.copy, data=nq_bf, bias=0.0, scale=1.0)
